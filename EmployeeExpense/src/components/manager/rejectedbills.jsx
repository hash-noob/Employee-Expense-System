// import React, { useEffect, useState } from 'react';
// import axios from 'axios';

// const RejectedBills = () => {
//     const [bills, setBills] = useState([]);
//     const [error, setError] = useState(null);

//     useEffect(() => {
//         const fetchRejectedBills = async () => {
//             try {
//                 const response = await axios.get('http://localhost:3001/api/manager/rejected-bills', {
//                     headers: {
//                         'Authorization': `Bearer ${localStorage.getItem('token')}`
//                     }
//                 });
//                 setBills(response.data);
//             } catch (error) {
//                 setError(error);
//             }
//         };

//         fetchRejectedBills();
//     }, []);

//     if (error) {
//         return <div className="text-red-500">Error: {error.message}</div>;
//     }

//     return (
//         <div className="container mx-auto mt-10">
            
//             {bills.length === 0 ? (
//                 <div>No rejected bills found</div>
//             ) : (
//                 <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
//                     <thead className="bg-red-200">
//                     <tr>
//             <th
//               colSpan={4}
//               className="text-xl font-bold py-4 text-center border-b border-red-200">
//               Rejected Bills
//             </th>
            
//           </tr>
//           <hr></hr>
//                         <tr>
//                             <th className="py-2 px-4">Claim ID</th>
//                             <th className="py-2 px-4">Employee ID</th>
//                             <th className="py-2 px-4">Comments</th>
//                             <th className="py-2 px-4">Bills</th>
//                         </tr>
//                     </thead>
//                     <tbody>
//                         {bills.map((bill) => (
//                             <tr key={bill._id} className="border-t">
//                                 <td className="border px-4 py-2">{bill.cId}</td>
//                                 <td className="border px-4 py-2">{bill.eId}</td>
//                                 <td className="border px-4 py-2">{bill.comments}</td>
//                                 <td className="border px-4 py-2">
//                                     <ul>
//                                         {bill.billsArray.map((b, index) => (
//                                             <li key={index}>
//                                                 Bill Id:{b.billId}
//                                             </li>
//                                         ))}
//                                     </ul>
//                                 </td>
//                             </tr>
//                         ))}
//                     </tbody>
//                 </table>
//             )}
//         </div>
//     );
// };

// export default RejectedBills;

// components/RejectedBills.jsx
import React, { useEffect, useState } from 'react';
import axios from 'axios';
import BillCard from './BillCard';

const RejectedBills = () => {
  const [bills, setBills] = useState([]);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchRejectedBills = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/manager/rejected-bills', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
        setBills(response.data);
      } catch (error) {
        setError(error);
      }
    };

    fetchRejectedBills();
  }, []);

  if (error) {
    return <div className="text-red-500">Error: {error.message}</div>;
  }

  return (
    <div className="container">
      <h2>Rejected Bills</h2>
      {bills.length === 0 ? (
        <div>No rejected bills found</div>
      ) : (
        <div className="card-container">
          {bills.map((bill) => (
            <BillCard
              key={bill._id}
              eId={bill.eId}
              title={bill.title}
              totalAmount={bill.totalAmount}
              datedFrom={bill.datedFrom}
              datedTo={bill.datedTo}
            />
          ))}
        </div>
      )}
    </div>
  );
};

export default RejectedBills;

